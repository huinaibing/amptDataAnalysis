...
- `analysisConfig.h` 负责配置读取和 PID/output metadata，`analysisUtils.h` 负责共享选择、中心度转换、GFW 填充和输入迭代，`corrConfigManager.h` 维护与 O2 GFW 对应的 region 和 correlation，`dataLoader.h`/`eventManager.h` 负责流式读取和按事件分组，`utils.h` 提供 FlowContainer 和 v2-pT-rho 填充工具。公共逻辑应留在这些头文件中，三个主入口只负责各自 workflow 的编排和输出。

## 与 O2 任务的接口契约

三个 ROOT 入口宏是 `/home/huinaibing/o2_workdir/O2Physics/PWGCF/Flow/Tasks/pidFlowPtCorr.cxx` 三个主要数据入口在 AMPT truth 数据上的一一对应实现。它们使用相同的分析量定义，并把结果写成 O2/AnalysisResults-compatible 的 ROOT 结构。顶层任务目录统一为 `pid-flow-pt-corr`；对应范围内的子目录、FlowContainer 名、profile/histogram 名、轴含义和 bootstrap 组织方式必须与 O2 主函数保持一致。同一个 downstream ROOT 后处理脚本应能按相同路径读取 O2 数据结果和 AMPT 结果。
...
AMPT truth 数据没有 reconstructed `sel8`、detector PID、track-quality、NUA/NUE 和 CCDB 校正。AMPT 入口使用 PDG code 区分 pion、kaon、proton，以单位粒子权重填充，并通过 impact parameter 转换中心度。“两边结构相同”指 ROOT 输出接口和物理量定义相同，不表示 detector-level 选择步骤或 event-count 每个 cut bin 的实际含义完全相同。AMPT 当前只生成适用于 truth 输入的 charged、pion、kaon、proton 结果，不生成 detector PID 失败对应的 unidentified 结果或 O2 QA/校正输出。
...
- 三个函数的共同参数依次是 input-file JSON 路径、输出 ROOT 路径、每个 JSON entry 最多读取的文件数、最多读取的 JSON entry 数、分析配置 JSON 路径；两个数量参数为 `-1` 时表示全部处理。`calculate_c22deltapt(...)` 在分析配置路径之前额外有一个 `usePure` 参数。
- `config/cent_cfg.json` 列出 AMPT 输入路径前缀和文件数量；`config/config.json` 管理 flow eta subevent、独立的 mean-pT eta 范围、各粒子 pT 范围、impact-parameter 中心度转换、随机种子、Pure/POI-ref 模式以及三个入口各自的输出轴。
- `v2_pt_rho_output`、`c22_delta_pt_output` 和 `pid_pt_correlations_output` 分别控制三个对应入口的边界策略和输出 axes。修改 binning 时要保证其 O2 对应输出及 downstream 脚本仍兼容。
